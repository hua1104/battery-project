...
<head>
    ...
    <style>
        ...
        .quick-date-btn { padding: 4px 10px; background: #334155; color: #e2e8f0; border: 1px solid #475569; border-radius: 4px; font-size: 12px; cursor: pointer; margin-left: 5px; }
        .quick-date-btn:hover { background: var(--accent); color: #0b1120; border-color: var(--accent); box-shadow: 0 0 0 2px rgba(56,189,248,0.15); }
        ...
        .btn-query:hover, .btn-action:hover {
            box-shadow: 0 6px 14px rgba(56, 189, 248, 0.35);
            transform: translateY(-1px);
        }
        .btn-del { background: #ef4444; color: white; padding: 5px 10px; border:1px solid #f87171; border-radius:4px; cursor:pointer; font-size:12px; box-shadow: 0 2px 6px rgba(239,68,68,0.25); }
        ...
        .btn-trash { background: #111827; border: 1px solid #64748b; color: #e2e8f0; width: 30px; height: 30px; border-radius: 4px; cursor: pointer; display: inline-flex; justify-content: center; align-items: center; transition: 0.2s; font-size: 16px; }
        .btn-trash:hover { border-color: var(--ng-color); color: #fff; background: rgba(239,68,68,0.2); }
        .view-toggle { display: flex; background: #0f172a; border-radius: 6px; padding: 2px; border: 1px solid #334155; }
        .view-btn { padding: 5px 12px; cursor: pointer; color: #cbd5f5; border-radius: 4px; font-size: 13px; }
        .view-btn.active { background: #334155; color: white; font-weight: bold; box-shadow: 0 0 0 1px rgba(148,163,184,0.4) inset; }
        ...
        .toggle-btn { padding: 6px 15px; border: 1px solid #475569; background: #0f172a; color: #cbd5f5; border-radius: 20px; cursor: pointer; font-size: 13px; }
        .toggle-btn.active { background: var(--accent); color: #0b1120; border-color: var(--accent); font-weight: bold; box-shadow: 0 0 0 2px rgba(56,189,248,0.15); }
        ...
        .tag { font-size: 12px; padding: 2px 6px; border-radius: 4px; border: 1px solid #334155; color: var(--text-sub); margin-left: 8px; }
        ...
        .gallery-select { position: absolute; top: 6px; left: 6px; background: rgba(15,23,42,0.85); border: 1px solid #334155; border-radius: 4px; padding: 2px; z-index: 5; }
...
<body>
    <div class="sidebar">
        <div class="brand">
            <h2 style="color:#38bdf8; margin:0;">锂电池检测结果管理平台</h2>
        ...
        <div class="nav-item" onclick="switchView('status', this)">🛠 系统状态</div>
    ...
    <div class="main">
        ...
        <div id="view-list" class="view-section">
            ...
            <div class="card">
                <div style="display:flex; gap:15px; flex-wrap:wrap; align-items:flex-end;">
                    ...
                    <div class="select-tools">
                        ...
                        <button class="btn-del" onclick="deleteSelected()" style="background:#ef4444;">🗑️ 删除选中</button>
                        <button class="btn-del" onclick="deleteBulk()" style="background:#ef4444;">🗑️ 批量删除 (所选时段)</button>
                    </div>
                    {% endif %}
                </div>
            </div>
            ...
            <div class="card" style="padding:12px; display:flex; justify-content:flex-end; gap:10px; align-items:center;">
                ...
                <span id="pageInfo" style="color:#94a3b8; font-size:12px;">1 / 1</span>
    ...
    <div class="lightbox" id="lightbox" onclick="if(event.target==this)this.style.display='none'">
        ...
        <div style="margin-top:15px; display:flex; gap:15px;">
            ...
            <button class="btn-query" style="background:#475569; color:white;" onclick="deleteRecord(document.getElementById('review_id').value); document.getElementById('lightbox').style.display='none';">🗑️ 删除此记录</button>
        </div>
        <div style="color:gray; margin-top:10px; font-size:12px;">* 点击上方按钮进行人工复核修正或删除</div>
    ...
    <script>
                ...
                if (data.success) {
                    ...
                    if (typeof echarts !== 'undefined') {
                        ...
                        const barOption = {
                            ...
                            xAxis: { type: 'category', data: data.chart.dates, axisLine: { lineStyle: { color: '#475569' } }, axisLabel: { color: '#94a3b8' } },
                        ...
                        const lineOption = {
                            ...
                            grid: { left: '3%', right: '4%', bottom: '3%', containLabel: true },
                            xAxis: { type: 'category', boundaryGap: false, data: data.chart.dates, axisLine: { lineStyle: { color: '#475569' } }, axisLabel: { color: '#94a3b8' } },
                            yAxis: { type: 'value', min: 0, max: 105, splitLine: { lineStyle: { color: '#334155' } }, axisLabel: { color: '#94a3b8' } },
                            series: [{
                    ...
                    data.data.forEach(r => { 
                        ...
                        tbody.innerHTML += `<tr>
                            ...
                            <td style="display:flex; gap:10px; align-items:center;">
                                <a href="javascript:void(0)" data-id="${r.id}" data-src="${imgSrc}" onclick="openLightbox(this)" style="color:#38bdf8">查看</a>
        ...
        async function loadStatus() {
            try {
                ...
                document.getElementById('st_db').style.color = s.db_ok ? '#10b981' : '#ef4444';
                ...
                if (items.length === 0) {
                    tbody.innerHTML = `<tr><td colspan="3" style="color:#94a3b8; text-align:center;">暂无异常记录</td></tr>`;
                } else {
                    items.forEach(e => {
        ...
        async function loadLogs() {
            ...
            try {
                ...
                if (items.length === 0) {
                    tbody.innerHTML = `<tr><td colspan="3" style="color:#94a3b8; text-align:center;">暂无日志</td></tr>`;
                    return;
                }
                items.forEach(line => {
