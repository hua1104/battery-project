...
DEFAULT_CONFIG = {
    ...
    "camera_width": 1280,
    "camera_height": 720,
    "camera_reopen_interval": 3,
    "upload_retry_interval": 30,
    "upload_max_retry": 3,
    "pending_file": "pending_uploads.jsonl",
    "log_dir": "logs",
    "offline_login_enabled": False,
    "offline_login_username": "admin",
    "offline_login_password": ""
}
...
def load_config(path=None):
    ...
    cfg_path = path or os.path.join(BASE_DIR, "config.json")
    try:
...
CONFIG = load_config()
CONFIG["model_path"] = resolve_path(CONFIG.get("model_path"))
CONFIG["pending_file"] = resolve_path(CONFIG.get("pending_file"))
CONFIG["log_dir"] = resolve_path(CONFIG.get("log_dir"))
CONFIG["save_folder"] = resolve_path(CONFIG.get("save_folder"))
...
CAMERA_REOPEN_INTERVAL = CONFIG["camera_reopen_interval"]
UPLOAD_RETRY_INTERVAL = CONFIG["upload_retry_interval"]
UPLOAD_MAX_RETRY = CONFIG["upload_max_retry"]
...
def setup_logging():
    try:
        ...
        log_path = os.path.join(CONFIG["log_dir"], "client.log")
...
def enqueue_pending(path, result_str, confidence, operator):
    record = {
        "id": _now_id(),
        "path": path,
        "result": result_str,
    ...
    try:
        ...
        logger.exception("Failed to enqueue pending upload")

def send_upload(path, result_str, confidence, operator):
    try:
        filename = os.path.basename(path)
        with open(path, "rb") as f:
            data = {"operator": operator, "result": result_str, "confidence": confidence}
            files = {"file": (filename, f, "image/jpeg")}
            resp = requests.post(f"{SERVER_URL}/upload", data=data, files=files, timeout=5)
...
def retry_pending_loop():
    while not sys_state["exit"]:
        try:
            ...
            sys_state["health"]["pending"] = len(pending)
            if pending:
                success_ids = set()
                for rec in pending:
                    if send_upload(rec.get("path"), rec.get("result"), rec.get("confidence"), rec.get("operator")):
                        success_ids.add(rec.get("id"))
                with pending_lock:
                    current = _read_pending_unlocked()
                    remaining = [r for r in current if r.get("id") not in success_ids]
                    if remaining:
            ...
            logger.exception("Retry pending uploads failed")
        time.sleep(UPLOAD_RETRY_INTERVAL)

def record_error(msg):
    try:
        sys_state["health"]["last_error"] = msg
        sys_state["health"]["last_error_time"] = time.strftime("%Y-%m-%d %H:%M:%S")
    except Exception:
        pass
def verify_login_on_server(username, password):
    try:
        ...
        sys_state["health"]["cloud"] = False
    if (
        CONFIG.get("offline_login_enabled")
        and username == CONFIG.get("offline_login_username")
        and password == CONFIG.get("offline_login_password")
    ):
        return True
    return False
...
def upload_task(path, result_str, confidence):
    start = time.time()
    success = False
    for i in range(UPLOAD_MAX_RETRY):
        if send_upload(path, result_str, confidence, sys_state.get("user")):
            success = True
            break
        time.sleep(0.5 * (i + 1))
    if success:
        ...
        enqueue_pending(path, result_str, confidence, sys_state.get("user"))
        record_error("上传失败")
        with lock:
            sys_state["log"] = f"[{time.strftime('%H:%M:%S')}] 上传失败，已加入补传队列"
...
def worker_thread():
    ...
    detector = None
    if AI_AVAILABLE:
        try: detector = DefectDetector(model_path=MODEL_PATH) 
        except: detector = None

    ai_ready = bool(detector and getattr(detector, "model", None))
    sys_state["health"]["ai"] = ai_ready
    if not ai_ready and not sys_state.get("ai_warned"):
        sys_state["ai_warned"] = True
        record_error("AI模型加载失败")
        with lock:
            sys_state["log"] = f"[{time.strftime('%H:%M:%S')}] AI模型加载失败，进入模拟模式"
    ...
    try:
        while not sys_state["exit"]:
            if cap is None or not cap.isOpened():
                now = time.time()
                if now - last_cam_try > CAMERA_REOPEN_INTERVAL:
                    try:
                        if cap: cap.release()
                    except Exception:
                        pass
            ...
            if ret:
                ...
                with lock: sys_state["live_frame"] = live_rgb
            else:
                frame = None
                fail_count += 1
                if fail_count >= 5:
            ...
            if detected and not last_detect:
                with lock: sys_state["log"] = f"[{time.strftime('%H:%M:%S')}] 传感器触发，图像采集..."
                time.sleep(0.8) 
                
                analyze_frame = frame.copy() if frame is not None else None
                if analyze_frame is None:
                    analyze_frame = np.zeros((720, 1280, 3), dtype=np.uint8)
                ...
                with lock:
                    ...
                    if is_bad: 
                        sys_state["bad"] += 1
                        if "褶皱" in res_str or "Wrinkle" in res_str: 
                            sys_state["defects"]["褶皱"] += 1; detail="褶皱"
                        elif "划痕" in res_str or "Scratch" in res_str: 
                            sys_state["defects"]["划痕"] += 1; detail="划痕"
                        else: 
                            sys_state["defects"]["其他"] += 1; detail="其他"
                    else: 
                        sys_state["good"] += 1
                ...
                threading.Thread(target=upload_task, args=(path, res_str, conf), daemon=True).start()
        ...
        record_error("worker_thread crashed")
    finally:
        try:
...
def start_worker():
    global worker_thread_handle
    with worker_lock:
        if worker_thread_handle and worker_thread_handle.is_alive():
            return
        worker_thread_handle = threading.Thread(target=worker_thread, daemon=True)
        worker_thread_handle.start()

def watchdog_thread():
    while not sys_state["exit"]:
        alive = False
        with worker_lock:
            alive = worker_thread_handle is not None and worker_thread_handle.is_alive()
        sys_state["health"]["worker"] = alive
        if not alive:
            logger.warning("Worker thread down, restarting")
            record_error("worker_thread restarted")
            start_worker()
        time.sleep(2)
...
class VirtualKeyboard(tk.Toplevel):
    def __init__(self, target_entry, root):
        ...
        # --- [布局优化] 主键盘顶部加入数字行，同时保留九宫格模式 ---
        self.layouts = {
            "en_lower": [
                ['1','2','3','4','5','6','7','8','9','0'],  # <--- 新增数字行
                ['q','w','e','r','t','y','u','i','o','p'],
                ['a','s','d','f','g','h','j','k','l'],
                ['z','x','c','v','b','n','m'],
                [',','.','@','/','-','_','+','=']
        ...
        self.btn_num = self.create_tab_btn(bar, "123", "numbers") # 九宫格入口
...
class StatusPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.lbl_disk.pack()
        self.lbl_ai = tk.Label(bot, text="AI模型: --", font=("微软雅黑", 12), bg=THEME["bg_panel"], fg="white")
        self.lbl_ai.pack()
        self.lbl_pending = tk.Label(bot, text="待补传: --", font=("微软雅黑", 12), bg=THEME["bg_panel"], fg="white")
        self.lbl_pending.pack()
        self.lbl_worker = tk.Label(bot, text="核心线程: --", font=("微软雅黑", 12), bg=THEME["bg_panel"], fg="white")
        self.lbl_worker.pack()
        self.lbl_last_error = tk.Label(bot, text="最后异常: --", font=("微软雅黑", 12), bg=THEME["bg_panel"], fg="white")
        self.lbl_last_error.pack()
    ...
    def update_data(self):
        ...
        self.lbl_ai.config(text=f"AI模型: {'在线' if h.get('ai') else '离线'}",
                           fg=THEME["ec_green"] if h.get("ai") else THEME["ec_red"])
        self.lbl_pending.config(text=f"待补传: {h.get('pending', 0)}")
        self.lbl_worker.config(text=f"核心线程: {'在线' if h.get('worker') else '离线'}",
                               fg=THEME["ec_green"] if h.get("worker") else THEME["ec_red"])
        last_err = h.get("last_error")
        last_time = h.get("last_error_time")
        if last_err:
            self.lbl_last_error.config(text=f"最后异常: {last_time} {last_err}", fg=THEME["ec_yellow"])
        else:
            self.lbl_last_error.config(text="最后异常: --", fg="white")
