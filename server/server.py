...
app = Flask(__name__)
app.secret_key = 'battery_copyright_final_2026_v5'

# ================= 1. 系统配置 (已适配阿里云宝塔) =================
DB_CONFIG = {
    'host': 'localhost',
    'user': 'root',
    'password': '6af6887566cc5fca',  # ✅ 这里已改为你的宝塔数据库密码 '3'
...
# ✅ 路径已改为 Linux 服务器专用路径
UPLOAD_FOLDER = '/www/wwwroot/received_images'
if not os.path.exists(UPLOAD_FOLDER):
    try:
        os.makedirs(UPLOAD_FOLDER)
        print(f"📁 图片存储目录已创建: {UPLOAD_FOLDER}")
    except Exception as e:
        print(f"❌ 创建目录失败 (请检查权限): {e}")
...
def upgrade_db_structure():
    ...
    try:
        ...
        # 1. 权限字段 (Role)
        try:
            cursor.execute("ALTER TABLE users ADD COLUMN role VARCHAR(20) DEFAULT 'operator'")
        except:
            pass

        # 确保 admin 是管理员 (默认密码设为123，如果存在)
        try:
            # 尝试插入默认管理员，如果存在则忽略
            default_pass = '123' 
        ...
        # 2. 置信度字段 (Confidence)
        try:
            cursor.execute("ALTER TABLE detection_records ADD COLUMN confidence FLOAT DEFAULT 0.95")
        except:
            pass

        # 3. 复核状态字段 (Is Reviewed)
        try:
            cursor.execute("ALTER TABLE detection_records ADD COLUMN is_reviewed TINYINT DEFAULT 0")
        except:
            pass
...
def upload():
    try:
        ...
        if client_conf:
            conf = float(client_conf)
        else:
            # 如果没有，生成一个随机数兜底 (模拟AI)
            conf = round(random.uniform(0.85, 0.999), 4)
        
        conn = get_db()
        c = conn.cursor()
        c.execute("INSERT INTO detection_records (operator, result, image_path, confidence) VALUES (%s, %s, %s, %s)", 
                  (request.form.get('operator'), request.form.get('result'), f"{today}/{fname}", conf))
        conn.commit()
        conn.close()
        return "OK", 200
    except Exception as e:
...
def search_history():
    try:
        ...
        if res_type == 'NG':
            sql += " AND result LIKE '%不合格%'"
        elif res_type == 'OK':
            sql += " AND result NOT LIKE '%不合格%'"
        elif res_type and res_type != 'All':
            sql += " AND result LIKE %s"
            params.append(f"%{res_type}%")
        
        sql += " ORDER BY detection_time DESC LIMIT 200"
        ...
        for r in results: 
            ...
            if current_conf is None:
                r['confidence'] = 95.0 # 默认值
            else:
                r['confidence'] = current_conf * 100
...
def dashboard_data():
    try:
        ...
        return jsonify({
            'success': True, 
            'stats': {'total': total, 'good': good, 'bad': bad, 'rate': rate}, 
            'chart': {'dates': c_dates, 'ok': c_ok, 'ng': c_ng, 'rate': c_rate}
        })
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)})
...
<head>
    ...
    <style>
        ...
        .modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 200; display: none; justify-content: center; align-items: center; backdrop-filter: blur(5px); }
        ...
        .view-toggle { display: flex; background: #0f172a; border-radius: 6px; padding: 2px; border: 1px solid #334155; }
        .view-btn { padding: 5px 12px; cursor: pointer; color: var(--text-sub); border-radius: 4px; font-size: 13px; }
        .view-btn.active { background: #334155; color: white; font-weight: bold; }
        ...
        .conf-bar-bg { width: 80px; height: 6px; background: #334155; border-radius: 3px; display: inline-block; margin-right: 5px; }
        ...
        .gallery-item { background: #111827; border: 1px solid #334155; border-radius: 8px; overflow: hidden; transition: 0.2s; position: relative; }
        .gallery-item:hover { transform: translateY(-5px); border-color: var(--accent); box-shadow: 0 10px 20px rgba(0,0,0,0.3); }
        .gallery-img { width: 100%; height: 120px; object-fit: cover; cursor: pointer; }
        .gallery-info { padding: 10px; font-size: 12px; color: var(--text-sub); display:flex; justify-content:space-between; }
        .gallery-tag { position: absolute; top: 5px; right: 5px; font-size: 10px; padding: 2px 6px; border-radius: 3px; font-weight: bold; }

        .lightbox { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); z-index: 300; display: none; justify-content: center; align-items: center; flex-direction: column; }
        .lightbox img { max-width: 90%; max-height: 80vh; border: 2px solid #334155; box-shadow: 0 0 50px rgba(0,0,0,0.8); }
        ...
        .disk-info { padding: 15px 25px; border-top: 1px solid #374151; font-size: 12px; color: #94a3b8; }
        .disk-bar-bg { height: 6px; background: #374151; border-radius: 3px; margin-top: 8px; overflow: hidden; }
...
<body>
    <div class="sidebar">
        <div class="brand">
            <h2 style="color:#38bdf8; margin:0;">锂电池检测管理平台</h2>
        ...
        <div style="margin-top:auto;">
            ...
            <div style="padding:20px;"><a href="/logout" style="color:#f87171; text-decoration:none; font-size:14px;">↺ 退出登录</a></div>
        </div>
    </div>
    ...
    <div class="main">
        <div id="view-dash" class="view-section active">
            <div class="top-bar">
                <h1>综合情况概览 <span style="font-size:12px; color:#10b981; margin-left:10px; border:1px solid #10b981; padding:2px 5px; border-radius:4px;">● AI 引擎运行中</span></h1>
            ...
            <div class="grid-4" id="stats-area">
                ...
                <div class="card"><div class="stat-label">AI 综合良率</div><div class="stat-val" style="color:#38bdf8" id="val_rate">-</div></div>
            </div>
        ...
        <div id="view-list" class="view-section">
            ...
            <div class="card">
                <div style="display:flex; gap:15px; flex-wrap:wrap; align-items:flex-end;">
                    ...
                    <div>
                        <label style="font-size:12px; color:#94a3b8;">缺陷类型</label>
                        <select id="q_res">
                            <option value="All">全部记录</option>
                            <option value="OK">仅合格 (OK)</option>
                            <option value="NG">所有不合格 (NG)</option>
                            <option value="Wrinkle">缺陷: 褶皱 (Wrinkle)</option>
                            <option value="Scratch">缺陷: 划痕 (Scratch)</option>
                        </select>
                    </div>
            ...
            <div class="card" style="padding:0; height:calc(100vh - 300px); overflow-y:auto;">
                <table id="historyTable">
                    <thead><tr><th>检测时间</th><th>操作员</th><th>AI 判定结果</th><th>AI 置信度</th><th>人工复核</th><th>操作</th></tr></thead>
    ...
    <div class="modal-overlay" id="userModal">
        <div class="modal">
            ...
            <div style="margin-bottom:15px;"><label style="color:#94a3b8; display:block; margin-bottom:5px;">登录账号</label><input type="text" id="modal_username" placeholder="请输入用户名"></div>
            <div style="margin-bottom:15px;"><label style="color:#94a3b8; display:block; margin-bottom:5px;">登录密码</label><input type="text" id="modal_password" placeholder="请输入密码 (留空则不修改)"></div>
            <div style="margin-bottom:25px;"><label style="color:#94a3b8; display:block; margin-bottom:5px;">权限身份</label><select id="modal_role"><option value="operator">普通操作员</option><option value="admin">系统管理员</option></select></div>
            <div style="display:flex; justify-content:flex-end; gap:10px;"><button onclick="closeModal()" style="background:#334155; border:none; color:white; padding:8px 15px; border-radius:5px; cursor:pointer;">取消</button><button onclick="saveUser()" class="btn-query">保存</button></div>
        </div>
    </div>
    ...
    <div class="lightbox" id="lightbox" onclick="if(event.target==this)this.style.display='none'">
        ...
        <div style="margin-top:15px; display:flex; gap:15px;">
            ...
            <button class="btn-query" style="background:#ef4444; color:white;" onclick="reviewRecord('不合格 (人工)')">❌ 修正为不合格</button>
        </div>
        <div style="color:gray; margin-top:10px; font-size:12px;">* 点击上方按钮进行人工复核修正</div>
    ...
    <script>
        ...
        window.onload = function() { 
            quickDate(14); 
            autoRefreshTimer = setInterval(() => { if(document.getElementById('view-dash').classList.contains('active')) loadDashboard(true); }, 30000);
        };
        ...
        async function loadDashboard(silent=false) {
            try {
                ...
                if (data.success) {
                    ...
                    if(data.stats.rate < 90) document.getElementById('val_rate').style.color = '#ef4444'; else document.getElementById('val_rate').style.color = '#38bdf8';
                    
                    if(barChart) barChart.destroy();
                    const ctx = document.getElementById('barChart').getContext('2d');
                    barChart = new Chart(ctx, { type: 'bar', data: { labels: data.chart.dates, datasets: [ { label: '良品', data: data.chart.ok, backgroundColor: '#10b981', stack: 'Stack 0' }, { label: '次品', data: data.chart.ng, backgroundColor: '#ef4444', stack: 'Stack 0' } ] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { grid: { display: false } }, y: { grid: { color: '#334155' }, stacked: true } } } });
                    
                    if(lineChart) lineChart.destroy();
                    const ctxLine = document.getElementById('lineChart').getContext('2d');
                    lineChart = new Chart(ctxLine, { type: 'line', data: { labels: data.chart.dates, datasets: [{ label: '良率 %', data: data.chart.rate, borderColor: '#38bdf8', borderWidth: 2, tension: 0.3, fill:true, backgroundColor:'rgba(56,189,248,0.1)' }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { min: 0, max: 105, grid: { color: '#334155' } } } } });
                }
            } catch(e) {}
        }
        ...
        async function loadHistory() {
            try {
                const res = await fetch('/api/search_history', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ start: document.getElementById('q_start').value, end: document.getElementById('q_end').value, res_type: document.getElementById('q_res').value, operator: document.getElementById('q_op').value }) });
                const data = await res.json();
                ...
                if(data.success) { 
                    data.data.forEach(r => { 
                        ...
                        const reviewStatus = r.is_reviewed ? '<span style="color:#10b981">已复核</span>' : '<span style="color:#94a3b8">未复核</span>';
                        ...
                        tbody.innerHTML += `<tr>
                            ...
                            <td>
                                <div style="display:flex; align-items:center;">
                                    <div class="conf-bar-bg"><div class="conf-bar-fill" style="width:${confVal}%; background:${confColor}"></div></div>
                                    <span style="font-size:12px; color:${confColor}">${confVal}%</span>
                                </div>
                            </td>
                            <td>${reviewStatus}</td>
                            <td><a href="javascript:void(0)" onclick="showLightbox('${r.id}', '${imgSrc}')" style="color:#38bdf8">复核/查看</a></td>
                        </tr>`;
                        
                        gallery.innerHTML += `<div class="gallery-item">
                            <span class="gallery-tag ${badgeClass}" style="background:${isNG?'#ef4444':'#10b981'}; color:black;">${r.result} ${confVal}%</span>
                            <img src="${imgSrc}" class="gallery-img" onclick="showLightbox('${r.id}', '${imgSrc}')">
                            <div class="gallery-info"><span>${r.detection_time.split(' ')[0]}</span><span>${reviewStatus}</span></div>
                        </div>`;
                    }); 
                }
            } catch(e) {}
        ...
        async function loadUsers() { const res = await fetch('/api/users'); const data = await res.json(); const tbody = document.getElementById('userBody'); tbody.innerHTML = ""; data.data.forEach(u => { const roleName = u.role === 'admin' ? '<span style="color:#38bdf8">系统管理员</span>' : '<span>普通操作员</span>'; tbody.innerHTML += `<tr><td>${u.username}</td><td>${roleName}</td><td><button class="btn-edit" onclick="openUserModal('${u.id}', '${u.username}', '${u.role}')">修改</button><button class="btn-del" onclick="deleteUser('${u.id}')">删除</button></td></tr>`; }); }
...
if __name__ == '__main__':
    upgrade_db_structure() # 启动时自动升级数据库
    print("🚀 软著版系统启动: http://0.0.0.0:5000")
    # ✅ 这里的 host='0.0.0.0' 是外网访问的关键
